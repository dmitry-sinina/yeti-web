...
--
-- Name: switch13; Type: SCHEMA; Schema: -; Owner: -
--
...
DECLARE
  ...
  v_end timestamp;
BEGIN
  set local search_path to switch13,sys,public;
  v_start:=now();
...
CREATE FUNCTION load_interface_out() RETURNS TABLE(varname character varying, vartype character varying, forcdr boolean, forradius boolean)
    ...
    AS $$
BEGIN
  RETURN QUERY SELECT "name","type","custom","for_radius" from switch13.switch_interface_out order by rank asc;
END;
$$;
...
DECLARE
  ...
  v_rel_suffix VARCHAR = '_release';
BEGIN
  PERFORM preprocess('switch13','route',false);
  PERFORM preprocess('switch13','process_dp',false);
  PERFORM preprocess('switch13','process_gw',false);
END;
$$;
...
CREATE FUNCTION tracelog(i_in class4.destinations) RETURNS void
    ...
    AS $$
BEGIN
  RAISE INFO'switch13.tracelog: % : %',clock_timestamp()::char(25),i_in;
END;
$$;
